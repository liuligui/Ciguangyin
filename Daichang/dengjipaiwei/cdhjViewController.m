...
@interface cdhjViewController ()<setcityDelegate>
{
    UIView *yincahngview;
    UIView *footvoew;
    UIView *topview;
...
@implementation cdhjViewController
...
-(void)InitControl{
    ...
    topview = [[UIView alloc] initWithFrame:CGRectMake(0,20, viewW, 80)];
    [topview setBackgroundColor:[UIColor whiteColor]];
    [self.view addSubview:topview];
    
    NSArray *titleArray = @[@"姓",@"填写人姓名"];
...
-(void) labelTouchUpInside:(UITapGestureRecognizer *)recognizer{
    ...
    if([label.text isEqualToString:@"请填写营业地址"])
    {
        sw = @"sw";
    }
    else
...
-(void)btnp{
    ...
    UITextField *Uyname = (UITextField *)[self.view viewWithTag:1001];
    ...
    NSString *string = [NSString stringWithFormat:@"%@%@",ServiceAddress,@"cgy/appRemoteServ/saveZgjz.do"];
    NSURL *url = [NSURL URLWithString:string];
    NSString *addres = lbaddress.text;
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [request setPostValue:[Globle shareInstance].user_id forKey:@"user_id"];
    [request setPostValue:name forKey:@"life_name"];
    [request setPostValue:namey forKey:@"life_tbr"];
