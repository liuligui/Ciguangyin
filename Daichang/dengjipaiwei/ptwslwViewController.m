...
@interface ptwslwViewController   ()<setcityDelegate>
{
    UIView *yincahngview;
    UIView *footvoew;
    UIView *topview;
...
@implementation ptwslwViewController
...
-(void)InitControl{
    ...
    NSArray *titleArray = @[@"亡者姓名",@"亡者地址",@"阳上人姓名"];
...
-(void) labelTouchUpInside:(UITapGestureRecognizer *)recognizer{
    ...
    if([label.text isEqualToString:@"请填写死亡地址"])
    {
        sw = @"sw";
    }
    else
...
-(bool)checknil
{
    UITextField *wname = (UITextField *)[self.view viewWithTag:1000];
    UITextField *Uyname = (UITextField *)[self.view viewWithTag:1002];
    
    
    UITextField *uitext2 = (UITextField *)[self.view viewWithTag:100000];
    UITextField *uitext3 = (UITextField *)[self.view viewWithTag:100002];
    NSString *name=wname.text;
    NSString *tbr =uitext2.text;
    NSString *phone =uitext3.text;
    NSString *namey = Uyname.text;

    if (name.length<1) {
        HUD.mode = MBProgressHUDModeText;
        [HUD setLabelText:@"请填写亡者姓名"];
        [HUD show:YES];
...
-(void)btnp{
    ...
    NSString *string = [NSString stringWithFormat:@"%@%@",ServiceAddress,@"cgy/appRemoteServ/savePtwslw.do"];
    NSURL *url = [NSURL URLWithString:string];
    NSString *addres = lbaddress.text;
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [request setPostValue:[Globle shareInstance].user_id forKey:@"user_id"];
    [request setPostValue:name forKey:@"die_name"];
    [request setPostValue:lbwaddress.text forKey:@"sale_address"];
    [request setPostValue:namey forKey:@"save_name"];
